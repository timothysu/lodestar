...
export function getSlotsSinceGenesis(config: ChainConfig, genesisTime: Number64): Slot {
  const diffInSeconds = Date.now() / 1000 - genesisTime;
  return Math.floor(diffInSeconds / config.SECONDS_PER_SLOT);
}

export function getCurrentSlot(config: ChainConfig, genesisTime: Number64): Slot {
...
export function computeTimeAtSlot(config: ChainConfig, slot: Slot, genesisTime: Number64): Number64 {
