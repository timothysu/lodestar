...
import {getStateBeaconCommittees} from "../../../../../../src/api/rest/beacon/state/getBeaconCommittees";
...
describe("rest - beacon - getStateBeaconCommittees", function () {
  ...
  it("should succeed without filters", async function () {
    ...
    beaconStateStub.getStateCommittees
      .withArgs("head")
      .resolves([{index: 0, slot: 1, validators: [1, 2, 3] as List<ValidatorIndex>}]);
  ...
  it("should succeed with filters", async function () {
    const beaconStateStub = ctx.rest.server.api.beacon.state as SinonStubbedInstance<BeaconStateApi>;
    beaconStateStub.getStateCommittees
      .withArgs("head", {slot: 1, epoch: 0, index: 10})
      .resolves([{index: 0, slot: 1, validators: [1, 2, 3] as List<ValidatorIndex>}]);

    const response = await supertest(ctx.rest.server.server)
      .get(urlJoin(BEACON_PREFIX, getStateBeaconCommittees.url.replace(":stateId", "head")))
      .query({slot: "1", epoch: "0", index: "10"})
      .expect(200)
      .expect("Content-Type", "application/json; charset=utf-8");
    expect((response.body as ApiResponseBody).data).to.not.be.undefined;
    expect((response.body as ApiResponseBody).data.length).to.be.equal(1);
  ...
  it("string slot", async function () {
    await supertest(ctx.rest.server.server)
      .get(urlJoin(BEACON_PREFIX, getStateBeaconCommittees.url.replace(":stateId", "head")))
      .query({slot: "1a", epoch: "0", index: "10"})
      .expect(400)
      .expect("Content-Type", "application/json; charset=utf-8");
  });
