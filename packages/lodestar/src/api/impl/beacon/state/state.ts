...
import {ContainerType, List, readonlyValues} from "@chainsafe/ssz";
...
export class BeaconStateApi implements IBeaconStateApi {
  ...
  async getStateRoot(stateId: StateId): Promise<Root | null> {
    ...
    return (this.config.getTypes(state.slot).BeaconState as ContainerType<allForks.BeaconState>).hashTreeRoot(state);
  }
